...
impl Gmrf {
    /// Default target relative error (one-sigma) used for RBMC variance estimates.
    pub const DEFAULT_RBMC_RELATIVE_ERROR: f64 = 0.1;
    ...
    pub fn rbmc_variances<R: Rng + ?Sized>(
        ...
        for _ in 0..num_samples {
            let probe = Vector::from_fn(dim, |_| rng.sample(StandardNormal));
    ...
    /// Compute the number of RBMC probe samples needed for a target relative error.
    ///
    ...
    pub fn rbmc_num_samples(target_rel_error: f64) -> Result<usize, GmrfError> {
    ...
    /// Approximate marginal variances with an RBMC probe count derived from a target error.
    ///
    /// Uses `DEFAULT_RBMC_RELATIVE_ERROR` when `target_rel_error` is `None`.
    pub fn rbmc_variances_with_error<R: Rng + ?Sized>(
        &mut self,
        target_rel_error: Option<f64>,
        rng: &mut R,
    ) -> Result<Vector, GmrfError> {
        let rel_error = target_rel_error.unwrap_or(Self::DEFAULT_RBMC_RELATIVE_ERROR);
        let num_samples = Self::rbmc_num_samples(rel_error)?;
        self.rbmc_variances(num_samples, rng)
    }
    ...
    fn exact_inverse_diag(&mut self) -> Result<Vector, GmrfError> {
        if let Some(q_factor) = &self.q_factor {
            ...
            for i in 0..n {
                ...
                rhs[i] = 1.0;
                q_factor.solve_in_place(&mut rhs)?;
                diag[i] = rhs[i];
            }
            return Ok(diag);
        }
...
mod tests {
    ...
    #[test]
    fn rbmc_samples_for_target_error() {
        let samples = Gmrf::rbmc_num_samples(0.1).unwrap();
        assert_eq!(samples, 200);
    }

    #[test]
    fn rbmc_variances_with_default_error_runs() {
        let precision = identity_precision(1);
        let mean = Vector::from_vec(vec![0.0]);
        let mut gmrf = Gmrf::from_mean_and_precision(mean, precision).unwrap();
        let mut rng = thread_rng();
        let variances = gmrf.rbmc_variances_with_error(None, &mut rng).unwrap();
        assert_eq!(variances.len(), 1);
        assert!(variances[0].is_finite());
    }

    #[test]
    fn rbmc_rejects_nonpositive_error() {
        let err = Gmrf::rbmc_num_samples(0.0).unwrap_err();
